...
class ExecutionStats:
    ...
    def print_stats(self):
        ...
        avg_period_ms = (
            stats["average_period"] * 1000 if stats["average_period"] is not None else 0
        )
        min_period_ms = (
            stats["min_period"] * 1000 if stats["min_period"] is not None else 0
        )
        max_period_ms = (
            stats["max_period"] * 1000 if stats["max_period"] is not None else 0
        )

        # Print execution period stats
        print(
            f"Ave: {stats['average_rate']:.2f} Hz ({avg_period_ms:.2f} ms), "
            f"Min: {stats['min_rate']:.2f} Hz ({min_period_ms:.2f} ms), "
            f"Max: {stats['max_rate']:.2f} Hz ({max_period_ms:.2f} ms)"
        )
        ...
        for label, times in self.exec_times.items():
            ...
            avg_time_ms = (
                exec_stats["average_time"] * 1000
                if exec_stats["average_time"] is not None
                else 0
            )
            min_time_ms = (
                exec_stats["min_time"] * 1000
                if exec_stats["min_time"] is not None
                else 0
            )
            max_time_ms = (
                exec_stats["max_time"] * 1000
                if exec_stats["max_time"] is not None
                else 0
            )
            print(
                f"  [{label}] Ave: {avg_time_ms:.2f} ms, Min: {min_time_ms:.2f} ms, Max: {max_time_ms:.2f} ms"
            )
...
def main(args=None):
    ...
    for _ in range(15):  # Simulate 15 periodic executions
        ...
        # Simulate timing of specific processes with the `with` statement
        with stats.time_block("process_1"):
            time.sleep(0.05)  # Simulate process execution time

        with stats.time_block("process_2"):
