...
class BicycleModel(SpatialBicycleModel):
    ...
    def linearize(self, v_ref, kappa_ref, delta_s, D_s=1, D_a=1):
        """
        Linearize the system equations around provided reference values.
        :param v_ref: velocity reference around which to linearize
        :param kappa_ref: kappa of waypoint around which to linearize
        :param delta_s: distance between current waypoint and next waypoint
        :param D_s: steering delay steps
        :param D_a: acceleration delay steps
         """

        nx = self.n_states + D_s + D_a  # 拡張された状態次元

        # 元の線形化
        f, A_lin, B_lin = self._linearize_dynamics(v_ref, kappa_ref, delta_s, D_s, D_a)

        # f を拡張
        f_aug = np.zeros(nx)
        f_aug[:self.n_states] = f  # 元の f を拡張したベクトルの先頭に配置

        # 拡張された A と B を計算
        A_aug = np.zeros((nx, nx))
        B_aug = np.zeros((nx, self.nu))

        # A_aug の埋め込み
        A_aug[:self.n_states, :self.n_states] = A_lin
        A_aug[:self.n_states, self.n_states:self.n_states + D_s] = self._compute_A_delay(v_ref, delta_s, D_s)
        # 遅延入力のシフト行列
        if D_s > 1:
            A_aug[self.n_states:self.n_states + D_s - 1, self.n_states + 1:self.n_states + D_s] = np.eye(D_s - 1)
        if D_a > 1:
            A_aug[self.n_states + D_s:self.n_states + D_s + D_a - 1, self.n_states + D_s + 1:self.n_states + D_s + D_a] = np.eye(D_a - 1)

        # B_aug の埋め込み
        B_aug[self.n_states + D_s - 1, 1] = 1.0  # ステアリング入力
        B_aug[self.n_states + D_s + D_a - 1, 0] = 1.0  # 加速度入力

        return f_aug, A_aug, B_aug

    def _linearize_dynamics(self, v_ref, kappa_ref, delta_s, D_s, D_a):
        """
        Linearize the dynamics without considering delays.
        """
        ...
        A_lin = np.stack((a_1, a_2, a_3), axis=0)
        B_lin = np.stack((b_1, b_2, b_3), axis=0)

        return f, A_lin, B_lin

    def _compute_A_delay(self, v_ref, delta_s, D_s):
        """
        Compute the partial derivatives of the dynamics with respect to delayed steering inputs.
        """
        A_delay = np.zeros((self.n_states, D_s))
        if v_ref != 0:
            A_delay[1, 0] = v_ref / self.length * delta_s
        return A_delay
